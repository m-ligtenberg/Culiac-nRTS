...
pub fn squad_management_system(
    ...
    mut unit_squad_query: Query<(Entity, &Unit, &Transform, Option<&mut TacticalState>, Option<&mut Communication>), With<Squad>>,
    time: Res<Time>,
) {
    ...
    for (squad_entity, mut squad) in squad_query.iter_mut() {
        ...
        squad.members.retain(|&member_entity| {
            unit_squad_query.get(member_entity).is_ok()
        });
...
fn create_faction_squads(
    ...
    if units.len() < 2 { return; } // Need at least 2 units for a squad
    ...
    for chunk in units.chunks(thread_rng().gen_range(3..=5)) {
        ...
        // Create squad entity
        let squad_entity = commands.spawn(Squad {
            id: squad_id_counter,
        ...
        play_tactical_sound("radio", &format!(
            "{:?} squad {} formed with {} members", 
            faction, squad_id_counter - 1, chunk.len()
        ));
    }
}
...
fn determine_squad_type(units: &[(Entity, &Unit, &Transform)], faction: Faction) -> SquadType {
    ...
    match faction {
        Faction::Cartel => {
            ...
            } else if unit_types.iter().any(|&t| matches!(t, UnitType::Enforcer | UnitType::HeavyGunner)) {
                SquadType::AssaultTeam
            } else if unit_types.iter().any(|&t| matches!(t, UnitType::Sniper | UnitType::Medic)) {
        ...
        Faction::Military => {
            if unit_types.iter().any(|&t| matches!(t, UnitType::SpecialForces | UnitType::Tank)) {
                SquadType::AssaultTeam
            } else if unit_types.iter().any(|&t| matches!(t, UnitType::Vehicle | UnitType::Helicopter | UnitType::Engineer)) {
                SquadType::SupportTeam
            } else {
                SquadType::ReconTeam
            }
...
fn update_squad_leadership(
    squad: &mut Squad,
    unit_query: &Query<(Entity, &Unit, &Transform, Option<&mut TacticalState>, Option<&mut Communication>), With<Squad>>,
) {
    ...
    if squad.leader.is_none() {
        ...
        for &member_entity in &squad.members {
            if let Ok((entity, unit, _, _, _)) = unit_query.get(member_entity) {
                if unit.health > 0.0 {
                    let leadership_score = unit.health + (unit.experience as f32 * 10.0) + (unit.kills as f32 * 5.0);
...
fn coordinate_recon_squad(
    ...
    let squad_center = calculate_squad_center(squad, unit_query);
    let advance_position = squad_center + Vec3::new(
...
fn calculate_flanking_position(squad_pos: Vec3, target_pos: Vec3) -> Vec3 {
    ...
    let direction = if thread_rng().gen_bool(0.5) { 1.0 } else { -1.0 };
...
pub fn communication_system(
    ...
    for (entity, transform, mut comm, tactical_state) in unit_query.iter_mut() {
        ...
        for (enemy_entity, enemy_transform, enemy_unit) in enemy_query.iter() {
            ...
            if distance < 150.0 && can_see_target(transform.translation, enemy_transform.translation) {
                let existing_contact = comm.known_enemies.iter_mut()
...
pub fn advanced_tactical_ai_system(
    ...
    for (entity, mut unit, transform, mut movement, mut tactical_state, communication, formation_opt) in unit_query.iter_mut() {
        if unit.health <= 0.0 { continue; }
...
fn analyze_tactical_situation(
    ...
    let nearby_enemies: Vec<&EnemyContact> = known_enemies.iter()
        .filter(|contact| contact.position.distance(unit_pos) < 200.0 && contact.confidence > 0.3)
        .collect();
    
    let closest_enemy_distance = nearby_enemies.iter()
...
fn execute_tactical_action(
    ...
    match action {
        TacticalAction::Advance(target) => {
            let advance_pos = current_pos + Vec3::new(
        ...
        TacticalAction::Retreat(target) => {
            let retreat_pos = current_pos + Vec3::new(
                thread_rng().gen_range(-80.0..80.0),
        ...
        TacticalAction::Regroup(_) => {
            let regroup_pos = current_pos + Vec3::new(
...
fn change_tactical_state(tactical_state: &mut TacticalState, new_state: TacticalMode, current_time: f32) {
...
fn update_psychological_state(tactical_state: &mut TacticalState, situation: &TacticalSituation, delta_time: f32) {
    // Update suppression level
    if situation.under_fire {
        tactical_state.suppression_level = (tactical_state.suppression_level + delta_time * 0.5).min(1.0);
    } else {
