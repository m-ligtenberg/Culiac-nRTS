    ...
    for mut operator in intel_operators.iter_mut() {
        if operator.intel_type == IntelType::RadioIntercept {
            ...
            if operator.intel_cooldown.finished() {
                ...
                operator.intel_cooldown =
                    Timer::from_seconds(rng.gen_range(5.0..15.0), TimerMode::Once);
    ...
    for mut operator in intel_operators.iter_mut() {
        if operator.intel_type == IntelType::Informant {
            ...
            if operator.intel_cooldown.finished() {
                ...
                operator.intel_cooldown =
                    Timer::from_seconds(rng.gen_range(20.0..60.0), TimerMode::Once);

                // Generate informant tip
                if rng.gen::<f32>() < 0.4 {
                    // 40% chance per check
                    if let Some(tip) =
                        generate_informant_tip(&military_units, &mut rng, time.elapsed_seconds())
                    {
                        intel_system
                            .global_intel_network
                            .informant_reports
        ...
        if operator.intel_type == IntelType::Reconnaissance {
            ...
            if operator.intel_cooldown.finished() {
                ...
                for (enemy_transform, enemy_unit) in enemy_units.iter() {
                    let distance = operator_transform
                        .translation
...
fn generate_terrain_intel(position: Vec3) -> TerrainIntel {
    ...
    TerrainIntel {
        cover_points: (0..rng.gen_range(2..6))
            .map(|_| {
                position + Vec3::new(rng.gen_range(-30.0..30.0), 0.0, rng.gen_range(-30.0..30.0))
            })
            .collect(),
        choke_points: (0..rng.gen_range(1..3))
            .map(|_| {
                position + Vec3::new(rng.gen_range(-50.0..50.0), 0.0, rng.gen_range(-50.0..50.0))
            })
            .collect(),
        ...
        escape_routes: (0..rng.gen_range(2..5))
            .map(|_| {
                position + Vec3::new(rng.gen_range(-60.0..60.0), 0.0, rng.gen_range(-60.0..60.0))
            })
            .collect(),
    }
}
...
pub fn counter_intel_system(
    ...
    for (military_entity, military_transform, _military_unit) in military_units.iter() {
        for (intel_entity, intel_transform, intel_operator) in intel_operators.iter() {
            let distance = military_transform
                .translation
                .distance(intel_transform.translation);
            ...
            if rng.gen::<f32>() < detection_chance * time.delta_seconds() * 0.1 {
                let alert = CounterIntelAlert {
                    threat_type: match intel_operator.intel_type {
                        IntelType::Reconnaissance => CounterIntelThreat::EnemyScout(intel_entity),
                        IntelType::RadioIntercept => {
                            CounterIntelThreat::RadioJamming(military_transform.translation, 100.0)
                        }
                        IntelType::Informant => {
                            CounterIntelThreat::InformantCompromised(intel_transform.translation)
                        }
                        IntelType::CounterIntel => {
                            CounterIntelThreat::SurveillanceDrone(military_entity)
                        }
                    },
...
pub fn process_intel_reports(
    ...
    for tip in &intel_system.global_intel_network.informant_reports {
        if matches!(tip.urgency, TipUrgency::Critical | TipUrgency::High)
            && current_time - tip.time_received < 60.0
        {
            // Fresh intel within 1 minute
            match &tip.tip_type {
...
pub fn intel_ui_system(
    ...
    // Create intel panel
    let recent_intercepts = intel_system
        .global_intel_network
        .active_intercepts
        .iter()
        .rev()
