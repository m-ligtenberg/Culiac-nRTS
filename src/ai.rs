...
pub fn ai_director_system(
    ...
    let current_performance = (unit_ratio * 0.3 + kill_ratio * 0.3 + ovidio_factor * 0.3 + time_factor * 0.1).clamp(0.0, 1.0);
    
    // Smooth performance tracking with exponential moving average
    ai_director.player_performance = ai_director.player_performance * 0.85 + current_performance * 0.15;
    ...
    let should_spawn = check_spawn_conditions(&ai_director, &game_state, cartel_units, military_units);
    
    if should_spawn {
        let spawn_result = execute_dynamic_spawning(&mut commands, &ai_director, &game_assets, &game_state);
...
pub fn unit_ai_system(
    ...
    for (mut unit, transform, mut movement) in unit_query.iter_mut() {
        ...
        match unit.faction {
            Faction::Military => {
                let behavior = choose_military_behavior(&unit, transform, &cartel_positions, ovidio_position);
                execute_military_behavior(&mut movement, transform, behavior, &cartel_positions);
            },
            Faction::Cartel => {
                let behavior = choose_cartel_behavior(&unit, transform, &military_positions, ovidio_position);
...
fn choose_military_behavior(
    ...
    let primary_target = if let Some(ovidio_pos) = ovidio_position {
        ovidio_pos
    } else if let Some(&closest_cartel) = cartel_positions.iter()
    ...
    let nearby_enemies = cartel_positions.iter()
...
fn choose_cartel_behavior(
    ...
    let nearest_threat = military_positions.iter()
        .min_by_key(|&&pos| (unit_pos.distance(pos) * 1000.0) as i32)
        .copied();
    
    let nearby_enemies = military_positions.iter()
...
fn find_retreat_position(unit_pos: Vec3, threat_positions: &[Vec3]) -> Vec3 {
    ...
    let closest_threat = threat_positions.iter()
        .min_by_key(|&&pos| (unit_pos.distance(pos) * 1000.0) as i32)
        .unwrap();
...
fn find_safest_position(unit_pos: Vec3, threat_positions: &[Vec3]) -> Vec3 {
    ...
    for i in 0..8 {
        ...
        let test_pos = unit_pos + Vec3::new(
            angle.cos() * 100.0,
            angle.sin() * 100.0,
            0.0,
        );
...
fn avoid_enemy_clusters(desired_pos: Vec3, enemy_positions: &[Vec3], avoidance_radius: f32) -> Vec3 {
...
fn check_spawn_conditions(ai_director: &AiDirector, game_state: &GameState, cartel_units: usize, military_units: usize) -> bool {
    ...
    let phase_trigger = matches!(game_state.game_phase, GamePhase::ApplyPressure | GamePhase::HoldTheLine);
...
fn execute_dynamic_spawning(commands: &mut Commands, ai_director: &AiDirector, game_assets: &Res<GameAssets>, game_state: &GameState) -> SpawnResult {
    ...
    for (i, position) in spawn_positions.iter().enumerate() {
        ...
        spawn_unit(commands, unit_type, Faction::Military, *position, game_assets);
    }
...
pub fn difficulty_settings_system(
    mut ai_director: ResMut<AiDirector>,
    ...
    if input.just_pressed(KeyCode::D) {
        ...
        let status = if ai_director.adaptive_difficulty { "ENABLED" } else { "DISABLED" };
