...
pub fn update_pooled_particles_system(
    ...
    for (entity, mut transform, mut particle, velocity, mut visibility, sprite_opt, text_opt) in particle_query.iter_mut() {
        if !particle.active {
            continue;
        }
...
pub fn spawn_optimized_combat_particles(
    ...
    for i in 0..num_particles {
        ...
        if let Some(entity) = particle_pool.get_or_spawn_combat_particle(
            commands, 
            position, 
            velocity, 
            0.8
            ...
            if let Ok(mut particle_query) = commands.get_entity(entity) {
...
pub fn setup_particle_pool(mut commands: Commands) {
    ...
    info!("🎭 Particle pool system initialized (max: {} particles)", ParticlePool::default().max_pool_size);
}