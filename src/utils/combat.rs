...
pub fn calculate_damage_modifier(weapon_type: &WeaponType) -> f32 {
    match weapon_type {
        ...
        WeaponType::TacticalRifle
        | WeaponType::CartelSniperRifle
        | WeaponType::MilitarySniperRifle => 1.3,
    ...
    if target_died {
        if let Ok((_, mut attacker_unit, _)) = unit_query.get_mut(attacker) {
            ...
            play_tactical_sound(
                "radio",
                &format!(
                    "{:?} gains experience from elimination",
                    attacker_unit.unit_type
                ),
            );
        }
    }
...
pub fn clear_invalid_targets(unit_query: &mut Query<(Entity, &mut Unit, &Transform)>) {
    let living_entities: std::collections::HashSet<Entity> = unit_query
        .iter()
...
pub fn spawn_combat_particles(commands: &mut Commands, attacker_pos: Vec3, target_pos: Vec3) {
    ...
    for i in 0..num_particles {
        ...
        let velocity = direction * thread_rng().gen_range(50.0..150.0)
            + Vec3::new(
