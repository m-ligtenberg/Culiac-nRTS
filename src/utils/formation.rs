use bevy::prelude::*;
use crate::components::{FormationType, UnitType, Faction};
...
pub fn calculate_formation_position(
    ...
    let position_offset = match formation_type {
        FormationType::Line => {
            let spacing = 40.0;
            let offset_x = (position_in_formation as f32 - (unit_count as f32 - 1.0) * 0.5) * spacing;
            Vec3::new(offset_x, 0.0, 0.0)
        },
        FormationType::Circle => {
            let radius = 60.0;
            let angle = (position_in_formation as f32 / unit_count as f32) * std::f32::consts::PI * 2.0;
        ...
        FormationType::Flanking => {
            let side = if position_in_formation % 2 == 0 { -1.0 } else { 1.0 };
...
pub fn calculate_flanking_position(
    ...
    let right_obstacle_count = obstacles.iter()
        .filter(|&&pos| right_pos.distance(pos) < 80.0)
        .count();
    let left_obstacle_count = obstacles.iter()
...
pub fn get_default_formation_type(unit_type: &UnitType, faction: &Faction, context: &str) -> FormationType {
    match context {
...
pub fn calculate_formation_facing(
    ...
    if let Some(objective) = objective_pos {
        ...
        let nearest_threat = threat_positions.iter()
