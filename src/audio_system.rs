...
pub fn setup_audio_system(
    ...
    // Load combat sounds
    audio_manager.combat_sounds.insert("gunfire_pistol".to_string(), 
        asset_server.load("audio/combat/gunfire_pistol.ogg"));
    audio_manager.combat_sounds.insert("gunfire_rifle".to_string(), 
        asset_server.load("audio/combat/gunfire_rifle.ogg"));
    audio_manager.combat_sounds.insert("gunfire_machinegun".to_string(), 
        asset_server.load("audio/combat/gunfire_machinegun.ogg"));
    audio_manager.combat_sounds.insert("explosion_small".to_string(), 
        asset_server.load("audio/combat/explosion_small.ogg"));
    audio_manager.combat_sounds.insert("explosion_large".to_string(), 
        asset_server.load("audio/combat/explosion_large.ogg"));
    audio_manager.combat_sounds.insert("vehicle_engine".to_string(), 
        asset_server.load("audio/combat/vehicle_engine.ogg"));
    audio_manager.combat_sounds.insert("helicopter".to_string(), 
        asset_server.load("audio/combat/helicopter.ogg"));
    audio_manager.combat_sounds.insert("reload".to_string(), 
        asset_server.load("audio/combat/reload.ogg"));
    audio_manager.combat_sounds.insert("ricochet".to_string(), 
        asset_server.load("audio/combat/ricochet.ogg"));
    
    // Load UI sounds
    audio_manager.ui_sounds.insert("button_click".to_string(), 
        asset_server.load("audio/ui/button_click.ogg"));
    audio_manager.ui_sounds.insert("button_hover".to_string(), 
        asset_server.load("audio/ui/button_hover.ogg"));
    audio_manager.ui_sounds.insert("menu_open".to_string(), 
        asset_server.load("audio/ui/menu_open.ogg"));
    audio_manager.ui_sounds.insert("menu_close".to_string(), 
        asset_server.load("audio/ui/menu_close.ogg"));
    audio_manager.ui_sounds.insert("notification".to_string(), 
        asset_server.load("audio/ui/notification.ogg"));
    audio_manager.ui_sounds.insert("warning".to_string(), 
        asset_server.load("audio/ui/warning.ogg"));
    audio_manager.ui_sounds.insert("victory".to_string(), 
        asset_server.load("audio/ui/victory.ogg"));
    audio_manager.ui_sounds.insert("defeat".to_string(), 
        asset_server.load("audio/ui/defeat.ogg"));
    
    // Load ambient sounds
    audio_manager.ambient_sounds.insert("city_ambience".to_string(), 
        asset_server.load("audio/ambient/city_ambience.ogg"));
    audio_manager.ambient_sounds.insert("wind".to_string(), 
        asset_server.load("audio/ambient/wind.ogg"));
    audio_manager.ambient_sounds.insert("distant_sirens".to_string(), 
        asset_server.load("audio/ambient/distant_sirens.ogg"));
    audio_manager.ambient_sounds.insert("crowd_panic".to_string(), 
        asset_server.load("audio/ambient/crowd_panic.ogg"));
    
    // Load radio chatter
    audio_manager.radio_sounds.insert("radio_static".to_string(), 
        asset_server.load("audio/radio/radio_static.ogg"));
    audio_manager.radio_sounds.insert("radio_beep".to_string(), 
        asset_server.load("audio/radio/radio_beep.ogg"));
    audio_manager.radio_sounds.insert("radio_voice_cartel".to_string(), 
        asset_server.load("audio/radio/radio_voice_cartel.ogg"));
    audio_manager.radio_sounds.insert("radio_voice_military".to_string(), 
        asset_server.load("audio/radio/radio_voice_military.ogg"));
    
    // Load background music
    audio_manager.background_music.insert("menu_theme".to_string(), 
        asset_server.load("audio/music/menu_theme.ogg"));
    audio_manager.background_music.insert("battle_theme".to_string(), 
        asset_server.load("audio/music/battle_theme.ogg"));
    audio_manager.background_music.insert("tension_theme".to_string(), 
        asset_server.load("audio/music/tension_theme.ogg"));
    audio_manager.background_music.insert("victory_theme".to_string(), 
        asset_server.load("audio/music/victory_theme.ogg"));
    audio_manager.background_music.insert("defeat_theme".to_string(), 
...
pub fn play_spatial_sound(
    ...
    if let Some(handle) = sound_handle {
        ...
        println!("{} [{}] Playing: {} at {:.1}, {:.1}", icon, sound_type.to_uppercase(), sound_name, position.x, position.y);
    } else {
...
pub fn queue_radio_message(
    ...
    if let Ok(mut radio_player) = radio_player_query.get_single_mut() {
        ...
        let insert_pos = radio_player.message_queue.iter()
...
pub fn background_music_system(
    ...
    if let Ok(mut music_player) = music_player_query.get_single_mut() {
        ...
        let desired_track = match game_state.game_phase {
            ...
            GamePhase::Preparation | GamePhase::InitialRaid | GamePhase::BlockConvoy | GamePhase::ApplyPressure | GamePhase::HoldTheLine => "battle_theme",
    ...
    match sound_type {
        "gunfire" => {
            ...
            if let Some(pos) = position {
                play_spatial_sound(commands, audio_manager, audio, "combat", sound_name, pos, 0.8);
            } else {
        ...
        "explosion" => {
            ...
            if let Some(pos) = position {
                play_spatial_sound(commands, audio_manager, audio, "combat", sound_name, pos, 1.0);
            } else {
                play_ui_sound(audio_manager, audio, sound_name);
            }
        },
        "vehicle" => {
            ...
            if let Some(pos) = position {
                play_spatial_sound(commands, audio_manager, audio, "combat", sound_name, pos, 0.7);
            } else {
                play_ui_sound(audio_manager, audio, sound_name);
            }
        },
