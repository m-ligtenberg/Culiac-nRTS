...
pub async fn initialize_database() -> Result<Pool<Sqlite>, AuthError> {
    let database_url = env::var("DATABASE_URL")
        .unwrap_or_else(|_| "sqlite:culiacan_rts.db".to_string());
    ...
    if !Sqlite::database_exists(&database_url).await.unwrap_or(false) {
        ...
        match Sqlite::create_database(&database_url).await {
            ...
            Err(error) => return Err(AuthError::DatabaseError(format!("Failed to create database: {}", error))),
        }
    }
...
async fn run_migrations(pool: &SqlitePool) -> Result<(), AuthError> {
    ...
    for (i, migration) in migrations.iter().enumerate() {
        ...
        sqlx::query(migration)
            ...
            .await
            .map_err(|e| {
                AuthError::DatabaseError(format!("Migration {} failed: {}", i + 1, e))
            })?;
    }
...
pub async fn cleanup_expired_data(pool: &SqlitePool) -> Result<(), AuthError> {
    ...
    let expired_sessions = sqlx::query("DELETE FROM sessions WHERE expires_at < datetime('now')")
        .execute(pool)
        .await
        .map_err(|e| AuthError::DatabaseError(format!("Failed to clean expired sessions: {}", e)))?;

    println!("Cleaned up {} expired sessions", expired_sessions.rows_affected());

    // Clean up expired password resets
    let expired_resets = sqlx::query("DELETE FROM password_resets WHERE expires_at < datetime('now')")
        .execute(pool)
        .await
        .map_err(|e| AuthError::DatabaseError(format!("Failed to clean expired password resets: {}", e)))?;

    println!("Cleaned up {} expired password resets", expired_resets.rows_affected());

    // Clean up expired email verifications
    let expired_verifications = sqlx::query("DELETE FROM email_verifications WHERE expires_at < datetime('now')")
        .execute(pool)
        .await
        .map_err(|e| AuthError::DatabaseError(format!("Failed to clean expired email verifications: {}", e)))?;

    println!("Cleaned up {} expired email verifications", expired_verifications.rows_affected());

    Ok(())
}

pub async fn create_admin_user(pool: &SqlitePool, username: &str, email: &str, password: &str) -> Result<(), AuthError> {
    use crate::auth::{CreateUserRequest, AuthDatabase, UserRole};
...
mod tests {
    ...
    async fn test_create_admin_user() {
        ...
        create_admin_user(&pool, "admin", "admin@example.com", "admin123").await.unwrap();
